...
class Synthesizer(nn.Module):
    ...
    def __init__(self, dag, name='synth', fixed_params={}):
        ...
        for processor, connections in self.dag:
            ...
            ext_params = [k for k, v in connections.items() if k not in self.processor_names+self.fixed_param_names]
            ext_details = {connections[k]:v for k,v in processor.get_param_sizes().items() if k in ext_params}
            self.ext_param_sizes.update(ext_details)
